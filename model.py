...
class StarGANVC(object):
    ...
    def build_model(self):
        ...
        self.discriminator_optimizer = tf.train.AdamOptimizer(learning_rate = self.discriminator_learning_rate, beta1 = 0.5).minimize(self.discrimator_loss, var_list = self.discriminator_vars)

        self.generator_optimizer = tf.train.AdamOptimizer(learning_rate = self.generator_learning_rate, beta1 = 0.5).minimize(self.generator_loss_all, var_list = self.generator_vars) 

        self.classifier_optimizer = tf.train.AdamOptimizer(learning_rate=self.classifier_learning_rate).minimize(self.domain_real_loss, var_list=self.classifier_vars)
